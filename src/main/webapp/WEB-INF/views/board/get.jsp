      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="col-lg-12 post-info">
            ...
            <div class="col-lg-2">
              <span class="post-info"><c:out value="${board.writer}" /></span><br>
            ...
            <div class="col-lg-9 text-right">
              <span class="post-info">조회수 <c:out value="${board.visit}"/></span> 
			...
			<div id="myModal" class="modal fade" role="dialog">
			  <div class="modal-dialog">
			    ...
			    <div class="modal-content">
			      ...
			      <div class="row">
					<div class="col-lg-6">
						받는 아이디 <input type="text" name="receiver" value='<c:out value="${board.writer}" />' class="form-control"/>
					</div>
					<div class="col-lg-6">
			...
			<!-- ============================================================추천하기=================================================================================== -->             	
	              <button type="button" class="btn btn-default" id="recomend" onclick="recommend('${id}');">
			...
			$.getJSON("/board/getAttachList", {bno: bno}, function(arr){
				...
				$(arr).each(function(i,attach){
					if(attach.fileType){
						...
						if(i===0){
							str += "<div class='item active'> <img src='/board/display?fileName="+path+"' style='width:100%;' ></div>"; 							
						}else{
							str += "<div class='item'> <img src='/board/display?fileName="+path+"' style='width:100%;' ></div>";	
						}						
					}
				});
				...
				$(arr).each(function(i,attach){
					if(attach.fileType){
						...
						list += "<img src='/board/display?fileName="+path+"'>";
						list += "</div>";
						list += "</li>";
					}
				});
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("button[data-oper='list']").on('click',function(e){
			actionForm.attr("action","/board/list").attr("method","get").empty().submit();
		});
	 ...
	 function showList(page){
		 ...
		 replyService.getList({bno:bnoValue, page:page||1}, function(replyCnt,list){
			 ...
			 for(var i=0, len=list.length; i<len; i++){
				// 댓글이 비공개 상태이나 로그인한 유저가 댓글 작성자 이거나 글 작성자만 댓글을 볼수있음
				 if(list[i].hide != 't' || loginUser==list[i].replayer || loginUser=="${board.writer}" ){ 
					 ...
					 str +="    <p id='Rereply'>"+list[i].reply+"</p><button id='Replyremove' type='button' data-oper='remove' data-rno='"+list[i].rno+"' data-id='"+list[i].replayer+"'>삭제</button>";
					/*  str +="      <button id='Replyadd'  type='button' data-oper='add' data-rno='"+list[i].rno+"' data-id='"+list[i].replayer+"'>답글 쓰기</button>"; */
					 str +="		</div></li>"; 
					 				
					console.log("비공개여부: "+list[i].hide);
					 //댓글이 비공개 상태이며, 다른유저가 댓글을 보았을때 비공개처리
					 ...
					 str +="  <div><div id='replyID'><strong class='primary-font' id='span_id'>"+list[i].replayer+"</strong>";
					 str +="   <small class='pull-right text-muted'>"+replyService.displayTime(list[i].replyDate)+"</small></div>";
					 str +="    <p id='Rereply'>비공개된 댓글입니다.</p><button id='Replyremove' type='button' data-oper='remove' data-rno='"+list[i].rno+"' data-id='"+list[i].replayer+"'>삭제</button>";
					/*  str +=" 			<button id='Replyadd' type='button' data-oper='add' data-rno='"+list[i].rno+"' data-id='"+list[i].replayer+"'>답글 쓰기</button>"; */
					 str +=" 			</div></li>"; // else if 문 추가.
				 }else{//위 조건이 해당되지 않는 이상 공개처리가 됨
					 str +=" <li id='rnoid' class='left claerfix' data-rno='"+list[i].rno+"'>"; //html문장 + 자바스크립트 + html태그 종료
					 str +="  <div><div id='replyID'><strong class='primary-font' id='span_id'>"+list[i].replayer+"</strong>";
					 str +="   <small class='pull-right text-muted'>"+replyService.displayTime(list[i].replyDate)+"</small></div>";
					 str +="    <p id='Rereply'>"+list[i].reply+"</p><button id='Replyremove' type='button' data-oper='remove' data-rno='"+list[i].rno+"' data-id='"+list[i].replayer+"'>삭제</button>";
	...
	$.getJSON("/board/getAttachList",{bno:bno}, function(arr){
		...
		$(arr).each(function(i,attach){
			...
			if(attach.fileType){
				...
				str +="<li data-path='"+attach.uploadPath+"' data-uuid='"+attach.uuid+"' data-name='"+attach.fileName+"' data-type='"+attach.fileType+"'>"
